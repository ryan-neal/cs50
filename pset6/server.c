...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char* query2 = NULL;
    //get absolute path and query
    size_t to_copy = strlen(request_target)+1;
    if (strstr(request_target, "?") != 0)
    {
        request_target = strtok(request_target, "?");
        size_t to_copy = strlen(request_target)+1;
        strncpy(abs_path, request_target, to_copy);
        query2 = strtok(NULL, " ");
        strcpy(query, query2);
    }
    else if (strstr(request_target, "q=") != 0)
    {
        request_target = strtok(request_target, "q=");
        size_t to_copy = strlen(request_target) + 1;
        strncpy(abs_path, request_target, to_copy);
        query2 = strtok(NULL, " ");
        strcpy(query, query2);
